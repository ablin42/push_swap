...
int		ps_rotate(t_ctrl **ctrl, t_node **stka, t_node **stkb)
{
	if ((*ctrl)->size_a >= 2 && (*ctrl)->size_b >= 2
	&& (*stka)->nb < (*stka)->next->nb && (*stkb)->nb < (*stkb)->next->nb)
		return (ps_cycle_move(ctrl, stka, stkb, RR));
	else if ((*ctrl)->size_a >= 2 && (*stka)->nb < (*stka)->next->nb)
		return (ps_cycle_move(ctrl, stka, stkb, RA));
...
/*	if (ps_getminnb(*ctrl, *stka, 0) == (*ctrl)->size_a - 1
	...
	&& ps_getmaxnb(*ctrl, *stka, 0) == 0)
		ps_cycle_move(ctrl, stka, stkb, RA);*///useless
	return (0);
}

int			ps_swap(t_ctrl **ctrl, t_node **stka, t_node **stkb)
{
	if ((*ctrl)->size_a >= 2 && (*ctrl)->size_b >= 2
	&& (*stka)->nb > (*stka)->next->nb && (*stkb)->nb > (*stkb)->next->nb)
		return (ps_cycle_move(ctrl, stka, stkb, SS));
	else if ((*ctrl)->size_a >= 2 && ps_next_number(*ctrl, *stka, (*stka)->next->nb, 0) == 0)
	{
		...
		return (ps_cycle_move(ctrl, stka, stkb, SB));
	return (0);
}

int			ps_magic(t_ctrl **ctrl, t_node **stka, t_node **stkb)
{
...
void		ps_sort(t_ctrl **ctrl, t_node **stka)
{
	...
	{
		i++;
		if (ps_swap(ctrl, stka, &stkb) == 0)
			if (ps_push(ctrl, stka, &stkb) == 0)
				if (ps_magic(ctrl, stka, &stkb) == 0)
					if (ps_rotate(ctrl, stka, &stkb) == 0)
